...
export default class VideoService {
    async getLatestVideos() {
        ...
        for (const id of this.channelsIds) {
            const req = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${id}&maxResults=1&order=date&key=${this.apiKey}`);
        ...
        return videos.reduce((prev: any, current: any) => {
            return (prev.snippet.publishedAt > current.snippet.publishedAt) ? prev : current
        });
    }
